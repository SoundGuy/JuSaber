...
namespace BSMG
{
    ...
    public class UIController : MonoBehaviour
    {
        ...
        private void GameplayLoop()
        {
            ...
            {
                float noteTime = note._time / currentSong.songInfo._beatsPerMinute * 60f + songStartTime -
                                 0 * currentSong.GetOffset();
                int keyMapIndex = note._lineLayer * 4 + note._lineIndex;
                string noteInput = keyMap[keyMapIndex];
                float notePrecision = hitPrecision / (currentSong.songInfo._beatsPerMinute * speedScale) * 60f;
                if (noteTime > Time.time - notePrecision && noteTime < Time.time + notePrecision * 2)
                {
                    // Visual timing
                    float frac = (Time.time - noteTime + notePrecision * 2f) / (notePrecision * 3f);
                    if (noteTime < Time.time + notePrecision && noteTime > Time.time - notePrecision)
                    {
                        // Active hitbox timing
                        if (Input.GetAxisRaw(noteInput) > 0.5 && note.NotYetHit() && !inputUsed[keyMapIndex])
                        {
                            // Make sure each key press counts for one note
                            Debug.Log("Note hit!");
                            note.Hit();
                            IncrementCombo();
                    ...
                    {
                        // Inactive hitbox timing (too early)
                        if (Input.GetAxisRaw(noteInput) > 0.5 && note.NotYetHit() && note.NotYetMissed() &&
                            !inputUsed[keyMapIndex])
                        {
                ...
                if (noteTime < Time.time - notePrecision && note.NotYetHit() && note.NotYetMissed())
                {
                    // Hitbox missed
        ...
        private void SelectSong()
        {
            ...
            Debug.Log("loaded  Songs" + loadedSongs + " index " + selectedSongIndex);
            Debug.Log("Selecting Song" + currentSong.GetName());
